...
export default {
  ...
  methods: {
    ...
    //   通过id号来获取指定栏目的文章数据
    async init () {
      const res = await getArticleList({ pageIndex: this.cateList[this.active].pageIndex, pageSize: this.cateList[this.active].pageSize, category: this.cateList[this.active].id })
      console.log(res)
      // 将数据存储到当前栏目的数组中
